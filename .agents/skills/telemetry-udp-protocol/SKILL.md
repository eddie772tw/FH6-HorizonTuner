...
- Axum REST/WebSocket 是獨立 HTTP/TCP 服務，開發模式固定為 `127.0.0.1:8001`；獨立後端可使用 `--port`。
- portable release 的 HTTP port 可能是動態值；讀取 `logs/web_port.txt` 或 sidecar readiness event。UDP port 仍依 telemetry 設定。
- `--scan --port 8000` 是 UDP 探針，不是 HTTP port。
...
- `backend-rust/src/runtime.rs` UDP 接收循環禁止同步檔案寫入、HTTP 請求與高開銷配置。
...
4. 執行 `cargo test --locked --manifest-path backend-rust/Cargo.toml`，若涉及前端資料流再執行 `cmd /c "pnpm -C frontend run test"`。
...
- 後端測試：`cargo test --locked --manifest-path backend-rust/Cargo.toml`
