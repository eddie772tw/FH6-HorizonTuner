...
def create_synthetic_324_packet() -> bytes:
    ...
    struct.pack_into(
        "<ffff", buf, 256, 185.0, 186.0, 190.0, 191.0
    )  # TireTemp F (256~271)
    ...
    struct.pack_into(
        "<BBBBB", buf, 303, 255, 0, 0, 0, 4
    )  # Accel=255, Brake=0, Clutch=0, HBK=0, Gear=4 (303~307)
...
def print_comparison_report(
    raw_data: bytes, source_label: str = "Synthetic Test Packet"
):
    print("=" * 80)
    ...
    print("-" * 80)
    print(
        f"{'欄位名稱':<22} | {'標準規範 (Spec Method A)':<28} | {'目前實作 (Code Method B)':<28}"
    )
    print("-" * 80)
    ...
    comparisons = [
        (
            "Position (X, Y, Z)",
            f"({spec['Position'][0]:.1f}, {spec['Position'][1]:.1f}, {spec['Position'][2]:.1f})",
            f"({code.get('Position', (0, 0, 0))[0]:.1f}, {code.get('Position', (0, 0, 0))[1]:.1f}, {code.get('Position', (0, 0, 0))[2]:.1f})",
        ),
        (
            "Speed (km/h)",
            f"{spec['Speed_Kmh']:.2f} km/h ({spec['Speed_Mps']:.1f} m/s)",
            f"{code.get('Speed_Mps', 0.0) * 3.6:.2f} km/h ({code.get('Speed_Mps', 0.0):.1f} m/s)",
        ),
        (
            "Power (HP)",
            f"{spec['Power_HP']:.1f} HP ({spec['Power_Watts']:.0f} W)",
            f"{code.get('Power_Watts', 0.0) / 745.7:.1f} HP ({code.get('Power_Watts', 0.0):.0f} W)",
        ),
        (
            "Torque (N·m)",
            f"{spec['Torque_Nm']:.1f} N·m",
            f"{code.get('Torque_Nm', 0.0):.1f} N·m",
        ),
        (
            "TireTemp (℉)",
            f"FL:{spec['TireTemp_F'][0]:.0f} FR:{spec['TireTemp_F'][1]:.0f} RL:{spec['TireTemp_F'][2]:.0f} RR:{spec['TireTemp_F'][3]:.0f}",
        ...
        (
            "Boost (psi)",
            f"{spec['Boost_PSI']:.2f} psi",
            f"{code.get('Boost_PSI', 0.0):.2f} psi",
        ),
        ("Fuel (0.0~1.0)", f"{spec['Fuel']:.2f}", f"{code.get('Fuel', 0.0):.2f}"),
        (
            "DistanceTraveled (m)",
            f"{spec['DistanceTraveled']:.1f} m",
            f"{code.get('DistanceTraveled', 0.0):.1f} m",
        ),
        (
            "BestLap (s)",
            f"{spec['BestLap']:.2f} s",
            f"{code.get('BestLap', 0.0):.2f} s",
        ),
        (
            "LastLap (s)",
            f"{spec['LastLap']:.2f} s",
            f"{code.get('LastLap', 0.0):.2f} s",
        ),
        (
            "CurrentLap (s)",
            f"{spec['CurrentLap']:.2f} s",
            f"{code.get('CurrentLap', 0.0):.2f} s",
        ),
        (
            "CurrentRaceTime (s)",
            f"{spec['CurrentRaceTime']:.1f} s",
            f"{code.get('CurrentRaceTime', 0.0):.1f} s",
        ),
        ("LapNumber", f"{spec['LapNumber']}", f"{code.get('LapNumber', 0)}"),
        ("RacePosition", f"{spec['RacePosition']}", f"{code.get('RacePosition', 0)}"),
        (
            "Accel / Brake Input",
            f"{spec['AccelInput']} / {spec['BrakeInput']}",
            f"{code.get('AccelInput', 0)} / {code.get('BrakeInput', 0)}",
        ),
        (
            "Gear / Steering",
            f"Gear {spec['Gear']} / Steering {spec['Steering']}",
            f"Gear {code.get('Gear', 0)} / Steering {code.get('SteerInput', 0)}",
        ),
    ]
    ...
    block3_items = [
        ...
        (
            ...
            f"{spec['NormalizedDrivingLine']:.2f}",
            "未解析 (Unparsed)",
        ),
    ...
    try:
        ...
        print(
            f"Spec (312-323): DeltaT={f_delta_t}, NormLine={f_norm_line}, PacketID={i_packet_id}"
        )
    ...
    try:
        ...
        print(
            f"Code (312-322): Lap={u_lap}, Pos={u_pos}, Accel={u_acc}, Brake={u_brk}, Clutch={u_clt}, HBK={u_hbk}, Gear={u_gear}, Steer={s_steer}, Line={s_line}, AIBrk={s_ai_brk}"
        )
    except Exception as e:
...
def scan_packets_for_tail_fields(port: int, count: int = 15, timeout: float = 5.0):
    ...
    for offset in range(0, min_len - 3):
        ...
        for idx in range(1, len(packets)):
            ...
            if curr_val != prev_val + 1:
                ...
                break
        if is_seq:
            packet_id_candidates.append(
                (offset, first_val, first_val + len(packets) - 1)
            )
    ...
    if packet_id_candidates:
        for off, start_v, end_v in packet_id_candidates:
            print(
                f"-> 發現候選 Offset {off} (0x{off:03X}): 數值 {start_v} -> {end_v} (精確 +1 遞增)"
            )
    else:
    ...
    print(
        f"註：由 TimestampMS (Offset 4) 計算得出的實際影格間隔平均值為: {avg_frame_time:.6f} 秒"
    )
    print("-" * 80)
    if deltat_candidates:
        for off, avg_v, sample_v in deltat_candidates:
            print(
                f"-> 發現候選 Offset {off} (0x{off:03X}): 平均值 {avg_v:.6f} s (範例: {sample_v:.6f} s)"
            )
    else:
        print(
            "  未在封包任何 Offset 發現數值界於 0.005s ~ 0.050s 之間的固定/動態 Float32 欄位。"
        )

    print("\n【探測結果 3：全封包末端 (232 ~ 末尾) Float32 欄位快照】")
    print("-" * 80)
    print(
        f"{'Offset':<10} | {'Float32 數值':<20} | {'Int32 數值':<15} | {'說明/備註':<25}"
    )
    print("-" * 80)
...
def main():
    parser = argparse.ArgumentParser(
        description="Forza Horizon Telemetry Block 2 & 3 Verification Tool"
    )
    parser.add_argument(
        "--live",
        action="store_true",
        help="開啟實時 UDP 監聽模式 (預設監聽 127.0.0.1:20440)",
    )
    parser.add_argument(
        "--scan",
        action="store_true",
            ...
            while True:
                ...
                if len(data) >= 232:
                    print_comparison_report(
                        data, source_label=f"Live UDP from {addr[0]}:{addr[1]}"
                    )
                    break
        except socket.timeout:
