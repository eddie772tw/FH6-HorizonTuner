...
- **發行包佈局與外置資源優先讀取 (Release Layout & External Lookup)**：重構 `build_release.bat` 自動部署產出至根目錄 `dist/FH6-HorizonTuner.exe`，並外置複製 `car_database.json` 與 `lang/`。重構 Rust `storage.rs` 的 `find_resource_file` 為「外置實體檔第一優先權 (External-First Lookup)」，使發行版使用者可直接開啟 `./lang/*.json` 與 `./car_database.json` 修改翻譯與獨立新增車輛庫。
- **舊版 JSON 資料結構無縫向下相容 (Legacy Compatibility)**：經評估與驗證，Rust 端的 `serde_json::Value` 能 100% 彈性解析與容錯舊版 Python/FastAPI 時代寫入在 `data/` 下的歷史數據（設定檔、HUD偏好與算牌紀錄），無需花費巨大代價改寫資料結構即可實現舊玩家無障礙升級。
