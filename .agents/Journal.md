...
**學習點 (Learning):**
1. **空氣阻力對極速影響之三次方根物理修正 (Aero Drag Velocity Cubic-Root Scaling)**：
   - 原模型對極速 $V_{top}$ 採用空力效率 $(E / 0.85)^1$ 一次方線性衰減，忽視了流體力學風阻功率 $P_{drag} = \frac{1}{2}\rho C_d A v^3 \implies v \propto (P / C_d A)^{1/3}$。這導致在高下壓力 (High Downforce) 設定下，$V_{top}$ 被嚴重低估 (降幅達 41%)，進而拉高終傳比與末檔齒比。
   - 重構後：改為三次方根修正 $\left(\frac{E}{0.85}\right)^{\frac{1}{3}}$，使高下壓力下極速衰減合理收斂於 15%~16% 範圍，精確對齊 Forza Horizon 遊戲遙測封包與實車終端極速。
2. **動力帶雙端漸進插值與末檔斷崖落差防護 (Geometric Progression & Cliff-Drop Protection)**：
   - 原模型一檔至前中段檔位固定乘以 $D \approx 0.75$，而最高檔由 $V_{top}$ 獨立推算，造成檔位在 1~5 檔密密麻麻聚集，6 檔驟降產生崖式落差 (Gear 6 / Gear 5 < 0.65)，升檔後轉速跌出最大扭力轉速 $RPM_T$。
   - 重構後：以動力帶自適應指數 $\gamma = 0.90 + 0.15(1 - r_{powerband})$ 的幾何曲線平滑連接 $G_1$ 至 $G_{N_{gears}}$，並引入末檔落差保護 $R_{N-1} \ge \max(0.76, 0.80 \times R_{N-2})$。在 2024 Ford Mustang Dark Horse (Car 3847, 882HP) 驗證中，6 檔齒比全數單調遞減、檔檔升檔落點均位於 5500~5833 RPM (高於 $RPM_T = 4750$)，徹底消除末檔落差陷阱。

**後續行動 (Action):**
- 後續若針對 Drag 或 Rally 齒比演算法調整時，亦應確保 $G_{i+1}/G_i$ 步階比滿足單調性與動力帶轉速下限防護。
