...
```text
FH6-HorizonTuner/
?ú‚??Ä .github/workflows/       # GitHub CI/CD workflow (Ruff Lint + Pytest)
?ú‚??Ä backend/                 # Python FastAPI backend core
??  ?ú‚??Ä main.py              # Backend entry point, API definitions & Overlay process management
??  ?ú‚??Ä telemetry_listener.py # UDP telemetry socket listener and parser
??  ?î‚??Ä car_database.json    # Built-in car database
?ú‚??Ä frontend/                # Tauri frontend code (Vite + React + TypeScript)
??  ?ú‚??Ä src/components/      # Frontend UI components
??  ??  ?ú‚??Ä TelemetryView.tsx    # Real-time telemetry dashboard
??  ??  ?ú‚??Ä OverlayView.tsx      # WYSIWYG dashboard layout editor
??  ??  ?ú‚??Ä TuningView.tsx       # Vehicle tuning workbench
??  ??  ?ú‚??Ä DragTestView.tsx     # Drag launch test
??  ??  ?ú‚??Ä AnalysisView.tsx     # Data analysis view
??  ??  ?ú‚??Ä DiagnosticConsole.tsx # Diagnostic log console
??  ??  ?î‚??Ä Navigation.tsx       # Navigation component
??  ?î‚??Ä src-tauri/           # Tauri window bundler configuration
?ú‚??Ä tool/                    # External native tooling
??  ?î‚??Ä overlay/             # C++ DXGI MPO Overlay rendering engine
??      ?ú‚??Ä main.cpp             # D3D11/ImGui data-driven rendering entry
??      ?ú‚??Ä DXGIOverlayManager.h/.cpp # DXGI swap chain management & MPO/fallback
??      ?ú‚??Ä WebSocketClient.h    # WinHTTP native WebSocket client
??      ?î‚??Ä CMakeLists.txt       # CMake build config (auto-fetch nlohmann/json, ExprTk, ImGui)
?ú‚??Ä lang/                    # Multi-language translation dictionaries (zh-tw, ja-jp, etc.)
?ú‚??Ä tests/                   # Pytest unit testing suite
?ú‚??Ä pyproject.toml           # Ruff formatting rules & Pytest configuration
?ú‚??Ä requirements.txt         # Python dependency list
?ú‚??Ä .pkgdirignore            # Package exclusion directory definitions
?ú‚??Ä start_all.bat            # One-click developer environment launcher
?î‚??Ä build_all.bat        # One-click standalone release bundler
```
...
Copyright (c) 2026 ÁΩêÈ†≠ (eddie772tw) & Contributors.
