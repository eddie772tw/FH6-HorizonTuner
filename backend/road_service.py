"""Road orchestration. Packet handling only updates memory and enqueues capture work."""
...
from road_models import CreateRoadCandidate, CreateRoadWorkflow, StartRoadRun
...
class RoadService:
    def __init__(self, database, store: RoadStore):
    ...
    def observe(self, data: dict) -> None:
        ...
        if identity and progressed:
            ...
            self.latest_car_class = int(car_class) if finite(car_class) else 0
        ...
        if identity is None or not progressed:
            return
        # This only checks that the driving clock has started. Data Out cannot
        # distinguish a Road event from free roam; event identity is user-confirmed.
    ...
    async def create(self, request: CreateRoadWorkflow) -> dict:
        workflow_id = uuid4().hex
        identity = request.identity.model_dump()
        if request.recommendation:
            await self.validate_recommendation(request.recommendation, identity)
        payload = {
            **request.model_dump(),
        ...
        setup = self.store.document(
            ...
            workflow_id,
            {
                ...
                "fields": {
                    ...
                    for key, value in request.recommendation.fields.items()
                }
                if request.recommendation
                else {},
                ...
                "formulaVersion": request.recommendation.formulaVersion
                if request.recommendation
                else None,
                "source": "recommendation" if request.recommendation else "unknown",
                "inputSnapshot": request.recommendation.inputSnapshot
                if request.recommendation
                else None,
    ...
    async def validate_recommendation(self, recommendation, identity=None) -> None:
        snapshot = recommendation.inputSnapshot
        observation = snapshot.get("engineObservation")
        if not isinstance(observation, dict) or not isinstance(
            observation.get("id"), str
        ):
            raise ValueError("The recommendation requires a saved engine observation")
        saved = await asyncio.to_thread(
            self.store.get, observation["id"], "engine-observation"
        )
        if saved["observation"] != observation or snapshot.get(
            "carId"
        ) != observation.get("carId"):
            raise ValueError(
                "The recommendation does not match its saved engine evidence"
            )
        if identity and (
            str(identity["ordinal"]) != observation["carId"]
            or identity["performanceIndex"]
            != observation["data"]["identity"]["performanceIndex"]
        ):
            raise ValueError(
                "Engine evidence belongs to a different car or configuration"
            )

    async def candidate(self, workflow_id: str, request: CreateRoadCandidate) -> dict:
        ...
        # Preserve A's original observation-only snapshot. The later game readback
        # is a separate, explicitly retrospective confirmation, never backdated.
        ...
        proposal = self.store.document(
            ...
            workflow_id,
            {
                **request.model_dump(),
                ...
                "methodVersion": "road-exploration/v1",
    ...
    async def start_run(self, workflow_id: str, request: StartRoadRun) -> dict:
        async with self.lock:
            if self.active:
                raise ValueError("Save the current Road run before starting another")
            ...
            if (
                recommendation
                and recommendation["inputSnapshot"] != request.inputSnapshot
            ):
                raise ValueError(
                    "Vehicle inputs changed. Create a new baseline and confirm the game settings again."
                )
            ...
            session_id = self.recorder.start_manual(
                car_ordinal=workflow["identity"]["ordinal"],
                car_name=workflow["carName"],
                car_class=self.latest_car_class,
                ...
                run = await asyncio.to_thread(
                    ...
                    workflow_id,
                    {
                        **request.model_dump(),
                        ...
                        "appliedSetup": {
                            key: {**field, "source": "game-confirmed"}
        ...
        if self.active and not self.recorder.is_recording and not self.lock.locked():
            async with self.lock:
                try:
                    ...
                    self.last_error = "The Road result could not be saved. Retry saving; captured data is retained."
    ...
    def _summarize(self, run_id: str, workflow_id: str, session_id: str):
        ...
        return self.store.append(
            ...
            workflow_id,
            {
                "runId": run_id,
                "sessionId": session_id,
                "observations": summarize_road_observations(points),
                "recording": metadata,
                "raceTimeCoverage": {
    ...
    async def recover(self):
        """Resume results after a process interruption; never resume capture silently."""
        self.persistence.start()
        documents = await asyncio.to_thread(self.store.list, exclude_capture=True)
