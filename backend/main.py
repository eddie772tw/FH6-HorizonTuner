        ...
        if data.get("IsRaceOn", 0) == 1:
            ...
            if now - self.last_sample_time >= self.downsample_interval:
                ...
                point = {
                    ...
                    "SuspTravel": list(data.get("NormalizedSuspensionTravel", [0.0, 0.0, 0.0, 0.0])),
                    "TireSlipAngle": list(data.get("TireSlipAngle", [0.0, 0.0, 0.0, 0.0])),
...
class DragRecorder:
    ...
    def record(self, data: dict):
        ...
        if self.status == "recording":
            ...
            # Condition C: Throttle release (excluding quick shifts)
                ...
                elif (timestamp_ms - self.low_throttle_start_time) / 1000.0 > self.low_throttle_duration_limit:
                    stop_recording = True
            ...
            if stop_recording:
                ...
                logger.info(f"Drag Test: Recording finished. Reason: {reason}. Total points: {len(self.current_session)}")
                self.analyze()
    ...
    def analyze(self):
        ...
        avg_front_slip = (sum(fl_slips) + sum(fr_slips)) / (2 * len(first_gear_pts)) if first_gear_pts else 0
        avg_rear_slip = (sum(rl_slips) + sum(rr_slips)) / (2 * len(first_gear_pts)) if first_gear_pts else 0
        ...
        launch_slip = avg_rear_slip if drivetrain == "RWD" else (avg_front_slip if drivetrain == "FWD" else (avg_front_slip + avg_rear_slip) / 2)
        
        launch_recommendation = ""
        if launch_slip > 0.18:
            launch_recommendation = "起步時驅動輪打滑過度（平均滑移率 {:.1f}%）。這會浪費抓地力，建議將 1 檔齒比調小（往 Speed 方向，數值調低 5%~10%）或調小終傳比，以降低輪胎端的瞬間起步扭力。".format(launch_slip * 100)
        elif launch_slip < 0.05:
            launch_recommendation = "起步時幾乎沒有打滑（平均滑移率 {:.1f}%）。若起步拉轉速度較慢，說明抓地力未被充分利用，建議將 1 檔齒比調大（往 Acceleration 方向，數值調高 5%~10%）以獲得更強的起步推力。".format(launch_slip * 100)
        else:
            launch_recommendation = "起步滑移率表現優異（平均滑移率 {:.1f}%），輪胎剛好處於最佳縱向抓地力區間（10%~15%）。請保持目前的 1 檔與終傳比設定。".format(launch_slip * 100)
        ...
        for i, p in enumerate(self.current_session):
            ...
            if current_gear is None:
                ...
                n_before = max(wp["CurrentEngineRpm"] for wp in window) if window else p["CurrentEngineRpm"]
                
                post_window = self.current_session[i:min(len(self.current_session), i+30)]
                throttle_pts = [wp for wp in post_window if wp["AccelInput"] > 200]
                n_after = min(wp["CurrentEngineRpm"] for wp in throttle_pts) if throttle_pts else (min(wp["CurrentEngineRpm"] for wp in post_window) if post_window else p["CurrentEngineRpm"])
                ...
                if s["retention"] < prev_s["retention"] - 0.02:
                    shift_recommendations.append(
                        "{} 檔升 {} 檔的轉速保留率（{:.1f}%）低於 {} 檔升 {} 檔（{:.1f}%）。這說明 {} 檔齒比相對於前一檔過疏，換檔後轉速掉得太深。建議將 {} 檔齒比調大（往 Acceleration 方向，數值調高 5%~8%）。".format(
                            s["from_gear"], s["to_gear"], s["retention"] * 100,
                            prev_s["from_gear"], prev_s["to_gear"], prev_s["retention"] * 100,
                            s["to_gear"], s["to_gear"]
                        )
                    )
                    ...
                    shift_recommendations.append(
                        "{} 檔升 {} 檔的齒比過密（轉速保留率高達 {:.1f}%）。這會導致頻繁換檔且無法充分拉長加速時間，建議將 {} 檔齒比調小（往 Speed 方向，數值調低 5%）。".format(
                            s["from_gear"], s["to_gear"], s["retention"] * 100, s["to_gear"]
                        )
                    )
            else:
                if s["retention"] < 0.62:
                    shift_recommendations.append(
                        "1 檔升 2 檔的轉速掉落過多（保留率僅 {:.1f}%）。建議將 2 檔齒比調大（往 Acceleration 方向，數值調高）以減小轉速落差，避免引擎掉出動力帶。".format(s["retention"] * 100)
                    )

        # 3. Final drive analysis
        last_pt = self.current_session[-1]
        max_rpm = max(p["CurrentEngineRpm"] for p in self.current_session)
        ...
        top_gear_max_rpm = max(p["CurrentEngineRpm"] for p in top_gear_pts) if top_gear_pts else 0
        
        if top_gear_max_rpm >= engine_max_rpm - 150:
            final_drive_recommendation = "車輛在最高檔位（{} 檔）達到了轉速紅線（{:.0f} RPM）。這限制了您的最高時速，建議將終傳比（Final Drive）調小（往 Speed 方向，數值降低 5%~10%）以釋放更高的極速潛力。".format(max_gear, top_gear_max_rpm)
        elif top_gear_max_rpm < engine_max_rpm * 0.72 and last_pt["SpeedMetersPerSecond"] > 0:
            last_1s_pts = [p for p in self.current_session if p["time"] > last_pt["time"] - 1.0]
            avg_accel = 0
            if len(last_1s_pts) > 1:
                dv = last_1s_pts[-1]["SpeedMetersPerSecond"] - last_1s_pts[0]["SpeedMetersPerSecond"]
                dt = last_1s_pts[-1]["time"] - last_1s_pts[0]["time"]
                avg_accel = dv / dt if dt > 0 else 0
                
            if avg_accel < 0.5:
                final_drive_recommendation = "測試結束時，最高檔位（{} 檔）的最高轉速僅為 {:.0f} RPM，且車輛已無明顯加速度。這說明終傳比過疏，引擎無法拉高轉速發揮馬力。建議將終傳比（Final Drive）調大（往 Acceleration 方向，數值提高 5%~10%）以提升加速響應。".format(max_gear, top_gear_max_rpm)
        
        if not final_drive_recommendation:
            final_drive_recommendation = "終傳比設定尚屬合理，最高檔位轉速與加速終點匹配良好。"
        ...
        if n_pts >= 10:
            ...
            num = sum((x_coords[i] - mean_x) * (z_coords[i] - mean_z) for i in range(n_pts))
            ...
            if den == 0:
                ...
                b = mean_z - a * mean_x
                denom = (a**2 + 1)**0.5
                deviations = [abs(a * x_coords[i] - z_coords[i] + b) / denom for i in range(n_pts)]
        ...
        active_pts = [p for p in self.current_session if p["Gear"] >= 1 and p["AccelInput"] > 200]
        stability_diagnostics = []
        avg_slip_diff = 0.0
        
        if active_pts:
            if drivetrain == "RWD":
                slip_diffs = [abs(p["TireSlipRatio"][2] - p["TireSlipRatio"][3]) for p in active_pts]
            elif drivetrain == "FWD":
                slip_diffs = [abs(p["TireSlipRatio"][0] - p["TireSlipRatio"][1]) for p in active_pts]
            else: # AWD
                slip_diffs = [(abs(p["TireSlipRatio"][0] - p["TireSlipRatio"][1]) + abs(p["TireSlipRatio"][2] - p["TireSlipRatio"][3])) / 2 for p in active_pts]
            ...
            if avg_slip_diff > 0.08:
                stability_diagnostics.append(
                    "偵測到驅動輪左右打滑嚴重失衡（平均滑移差值 {:.1f}%）。這通常是由於【差速器加速鎖定率 (Acceleration Lock)】過低所引發的單邊打滑（動力流失至空轉輪）。建議將差速器加速鎖定率調高 10%~20%，以確保兩側驅動輪獲得均衡扭力，維持加速軌跡穩定。".format(avg_slip_diff * 100)
                )
            ...
            # Diagnostic B: Over-locked Differential -> Fish-tailing (oscillation in slip difference and yaw)
                ...
                if total_leads > 10 and left_leads / total_leads > 0.25 and right_leads / total_leads > 0.25:
                    stability_diagnostics.append(
                        "偵測到車尾在加速過程中出現左右搖擺（蛇行，Fish-tailing，偏航角波動達 {:.1f}°）。這通常是由於【差速器加速鎖定率 (Acceleration Lock)】過高，限制了左右輪必要轉速差而產生強烈側向力矩。建議將差速器加速鎖定率降低 10%~15%，以提升行車穩定性。".format(math.degrees(yaw_variance_rad))
                    )
                    
            if not stability_diagnostics:
                if avg_slip_diff < 0.03 and yaw_variance_rad < 0.04:
                    stability_diagnostics.append("直行穩定性優異，左右動力分配非常均衡，加速時車身無明顯偏擺。")
                else:
                    stability_diagnostics.append("直行穩定性良好。加速過程中車身動態對稱。")
        ...
        self.car_name = car_database.get(str(self.car_id), {}).get("display_name", f"Car {self.car_id}")
        ...
        self.analysis_result = {
            ...
            "max_gear": max_gear,
            "max_speed_kmh": round(max(p["SpeedMetersPerSecond"] for p in self.current_session) * 3.6, 1) if self.current_session else 0.0,
        ...
        if car_id and car_id != "0":
            ...
            if app_settings.get("dyno_recording", True) and car_id in dyno_cache:
                ...
                if app_settings.get("dyno_filter_slip", True):
                    ...
                    SLIP_THRESHOLD = 0.10
                    if drivetrain == "RWD":
                        if abs(slip_ratios[2]) > SLIP_THRESHOLD or abs(slip_ratios[3]) > SLIP_THRESHOLD:
                            no_slip = False
                    elif drivetrain == "FWD":
                        if abs(slip_ratios[0]) > SLIP_THRESHOLD or abs(slip_ratios[1]) > SLIP_THRESHOLD:
                            no_slip = False
                    else:  # AWD or default
                        if any(abs(s) > SLIP_THRESHOLD for s in slip_ratios):
                            no_slip = False
                
                if (rpm > 0 and accel_input == 255 and gear > 0 and clutch_input == 0 and 
                        gear_match and no_braking and no_transient and no_slip):
...
async def drag_save_session():
    ...
    session_payload = {
        "metadata": {
            ...
            "launch_slip_percent": drag_recorder.analysis_result.get("launch_slip_percent", 0.0)
        },
