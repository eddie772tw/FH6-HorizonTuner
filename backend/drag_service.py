"""Drag workflow orchestration. Telemetry handling only updates memory and enqueues capture work."""
...
from drag_models import CreateDragCandidate, CreateDragWorkflow, StartDragRun
...
class DragService:
    ...
    async def create(self, request: CreateDragWorkflow) -> dict:
    ...
    async def candidate(self, workflow_id: str, request: CreateDragCandidate) -> dict:
        ...
        proposal = self.store.document(
            ...
            workflow_id,
            {
                **request.model_dump(),
                ...
                "methodVersion": "drag-exploration/v1",
    ...
    async def start_run(self, workflow_id: str, request: StartDragRun) -> dict:
        async with self.lock:
            if self.active:
                raise ValueError("Save the current Drag run before starting another")
        ...
        if self.active and not self.recorder.is_recording and not self.lock.locked():
            async with self.lock:
                try:
                    ...
                    self.last_error = "The Drag result could not be saved. Retry saving; captured data is retained."
    ...
    def _summarize(self, run_id: str, workflow_id: str, session_id: str):
        ...
        return self.store.append(
            ...
            workflow_id,
            {
                "runId": run_id,
                "sessionId": session_id,
                "observations": summarize_drag_observations(points),
                "recording": metadata,
            },
        )
