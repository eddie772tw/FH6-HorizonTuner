"""Drift workflow orchestration. Telemetry handling only updates memory and enqueues capture work."""
...
from drift_models import CreateDriftCandidate, CreateDriftWorkflow, StartDriftRun
...
class DriftService:
    ...
    def observe(self, data: dict) -> None:
        ...
        if not self.recorder.total_count and not (
            data.get("IsRaceOn") == 1 and finite(data.get("CurrentRaceTime"))
        ):
            return
        self.recorder.record(data)

    async def create(self, request: CreateDriftWorkflow) -> dict:
    ...
    async def candidate(self, workflow_id: str, request: CreateDriftCandidate) -> dict:
        ...
        proposal = self.store.document(
            ...
            workflow_id,
            {
                **request.model_dump(),
                ...
                "methodVersion": "drift-exploration/v1",
    ...
    async def start_run(self, workflow_id: str, request: StartDriftRun) -> dict:
        async with self.lock:
            if self.active:
                raise ValueError("Save the current Drift run before starting another")
        ...
        if self.active and not self.recorder.is_recording and not self.lock.locked():
            async with self.lock:
                try:
                    ...
                    self.last_error = "The Drift result could not be saved. Retry saving; captured data is retained."
    ...
    def _summarize(self, run_id: str, workflow_id: str, session_id: str):
        ...
        return self.store.append(
            ...
            workflow_id,
            {
                "runId": run_id,
                "sessionId": session_id,
                "observations": summarize_drift_observations(points),
                "recording": metadata,
            },
        )
