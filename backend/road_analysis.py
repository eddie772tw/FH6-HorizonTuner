...
def descriptive_debrief(points: list[dict]) -> dict:
    """Compatibility API fields carry observations, never a chassis diagnosis."""
    report = summarize_road_observations(points)
    weights, _ = observation_weights(points)
    wheel_reports = list(report["wheels"].values())
    means = [wheel["temperatureC"]["mean"] for wheel in wheel_reports]
    counts = [wheel["nearCompression"]["count"] for wheel in wheel_reports]
    travel = [
        value
        for p, weight in zip(points, weights)
        if weight > 0
        for w in range(4)
        if (value := wheel_value(p, "NormalizedSuspensionTravel", w)) is not None
    ]
    front = rear = corner = 0.0
    for point, weight in zip(points, weights):
        angle = [wheel_value(point, "TireSlipAngle", w) for w in range(4)]
        lateral, speed = point.get("AccelerationX"), point.get("SpeedMetersPerSecond")
        if (
            weight <= 0
            or not all(finite(v) for v in (*angle, lateral, speed))
            or abs(lateral) < 2.94
            or speed < 10
        ):
            continue
        corner += weight
        f, r = (abs(angle[0]) + abs(angle[1])) / 2, (abs(angle[2]) + abs(angle[3])) / 2
        if f > r * 1.15:
            front += weight
        elif r > f * 1.15:
            rear += weight
    return {
        ...
        "valid_laps": sum(lap["complete"] for lap in report["laps"]),
        "observed_laps": len(report["laps"]),
        "observed_seconds": report["quality"]["observedSeconds"],
        "tire_thermals": {
            **dict(zip(("fl_avg", "fr_avg", "rl_avg", "rr_avg"), means)),
            "status": "Observed"
        ...
        "suspension": {
            "peak_travel_pct": max(travel) * 100 if travel else None,
            # Historical key retained for API compatibility. v2 is a
            # count of continuous near-compression wheel events.
            "bottom_out_count": sum(v for v in counts if v is not None)
            if any(v is not None for v in counts)
            else None,
            "status": "Observed" if travel else "no_data",
        },
        "handling_balance": {
            "understeer_pct": front / corner * 100 if corner else None,
            "oversteer_pct": rear / corner * 100 if corner else None,
            "observed_seconds": corner,
            "tendency": "Observed normalized slip" if corner else "no_data",
        },
    }
