...
class RaceRecorder:
    ...
    def __init__(
        ...
        *,
        ...
        self._limit_race_time: float | None = None
    ...
    def record(self, data: dict[str, Any]) -> None:
        ...
        if self._limit_identity is not None:
            if (
                ...
                and (
                    self._limit_race_time is None
                    or current_race_time >= self._limit_race_time
                )
            ):
                return
            self._limit_identity = None
            self._limit_race_time = None
        if (
        ...
        if self.total_count >= self.max_samples:
            ...
            self._limit_race_time = current_race_time
            self.save_latest_and_clear("sample-limit")
            return
