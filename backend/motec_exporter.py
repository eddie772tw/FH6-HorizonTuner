...
def export_session_to_motec_csv(session_meta: Dict[str, Any], telemetry_points: List[Dict[str, Any]], output_filepath: str) -> bool:
    """
    ...
    try:
        ...
        with open(output_filepath, "w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow(["Device", "FH6 Horizon Tuner Telemetry", "Serial", "FH6-HORIZON-TUNER"])
            writer.writerow(["Date", session_id, "Time", "00:00:00"])
            writer.writerow(["Driver", "Driver", "Vehicle", car_name])
            writer.writerow(["Venue", "Forza Circuit", "Comment", f"Exported Telemetry Session {session_id}"])
            writer.writerow(["Sample Rate", "10.0"])
            writer.writerow([])
            
            # Channel Names Row
            headers = [
                "Time", "Distance", "Lap Number",
                "Speed", "Engine RPM", "Gear",
                "Throttle Pos", "Brake Pos", "Steer Pos",
                ...
                "Slip Ratio FL", "Slip Ratio FR", "Slip Ratio RL", "Slip Ratio RR",
                "Tire Temp FL", "Tire Temp FR", "Tire Temp RL", "Tire Temp RR"
            ]
            ...
            for p in telemetry_points:
                ...
                row = [
                    ...
                    f"{accel_x_g:.3f}",
                    f"{accel_z_g:.3f}",
                    f"{(susp[0] * 100):.1f}", f"{(susp[1] * 100):.1f}", f"{(susp[2] * 100):.1f}", f"{(susp[3] * 100):.1f}",
                    f"{(s_angle[0] * 57.29578):.2f}", f"{(s_angle[1] * 57.29578):.2f}", f"{(s_angle[2] * 57.29578):.2f}", f"{(s_angle[3] * 57.29578):.2f}",
                    f"{s_ratio[0]:.3f}", f"{s_ratio[1]:.3f}", f"{s_ratio[2]:.3f}", f"{s_ratio[3]:.3f}",
                    f"{((temp[0] - 32) * 5 / 9):.1f}", f"{((temp[1] - 32) * 5 / 9):.1f}", f"{((temp[2] - 32) * 5 / 9):.1f}", f"{((temp[3] - 32) * 5 / 9):.1f}"
                ]
                writer.writerow(row)
