import pytest
from backend.services.recorders import race_recorder, drag_recorder
...
def test_race_recorder_basic():
    ...
    race_recorder.record({"IsRaceOn": 0, "CurrentRaceTime": 0.0}, race_recording_enabled=True)
    assert not race_recorder.is_recording
    
    # enabled and race on
    race_recorder.record({"IsRaceOn": 1, "CurrentRaceTime": 1.0, "CurrentLap": 1, "TimestampMS": 1000}, race_recording_enabled=True)
    ...
    for i in range(55):
        ...
        race_recorder.record({"IsRaceOn": 1, "CurrentRaceTime": 1.0, "CurrentLap": 1, "TimestampMS": 1000 + i * 100}, race_recording_enabled=True)
...
def test_drag_recorder_basic():
    ...
    drag_recorder.record({"SpeedMetersPerSecond": 0.1, "Gear": 1, "AccelInput": 255, "TimestampMS": 1000, "IsRaceOn": 1})
    assert drag_recorder.status == "recording"
    
    # Continue recording
    drag_recorder.record({"SpeedMetersPerSecond": 10.0, "Gear": 1, "AccelInput": 255, "TimestampMS": 2000, "IsRaceOn": 1})
    assert len(drag_recorder.current_session) == 2
    
    # Stop recording by lifting throttle for a long time
    drag_recorder.record({"SpeedMetersPerSecond": 20.0, "Gear": 2, "AccelInput": 0, "TimestampMS": 3000, "IsRaceOn": 1})
    drag_recorder.record({"SpeedMetersPerSecond": 20.0, "Gear": 2, "AccelInput": 0, "TimestampMS": 4000, "IsRaceOn": 1})
