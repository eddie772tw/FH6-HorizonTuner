...
def test_sqlite_session_and_points(temp_sqlite_db):
    ...
    # Create session
    db.create_session(session_id=session_id, car_name="2020 Porsche 911 GT3", start_time=1000.0)
...
def test_motec_csv_export(temp_sqlite_db):
    ...
    db.create_session(session_id=session_id, car_name="Test Race Car", start_time=2000.0)
    ...
    try:
        ...
        success = export_session_to_motec_csv(session_meta, db.get_telemetry_points(session_id), csv_path)
