...
export function renderPowerTorque(data, powerTorqueHist, now) {
    ...
    if (powerTorqueHist.length < 300) {
        ...
        if (oldPT) { oldPT.rpm = currentRPM; oldPT.power = currentPwr; oldPT.torque = currentTq; oldPT.time = now; powerTorqueHist.push(oldPT); }
    }

    var ptCanvas = document.getElementById('tcPowerTorqueChart');
    if (ptCanvas && powerTorqueHist.length > 0) {
