...
import { configureBackendTransport, waitForBackendReady } from "../../src/services/backend";
...
async function initHudApp() {
  try {
    ...
    console.log("[HUD Frontend] Backend sidecar is ready on port:", backend.port);
  } catch (error) {
    console.error("[HUD Frontend] Backend startup failed:", error);
    const root = document.getElementById("root");
    if (root) {
      root.textContent = `Unable to connect to HUD backend. ${error instanceof Error ? error.message : String(error)}`;
    }
    return;
  }
