...
describe('SettingsSurface information architecture', () => {
  it('projects peer sections for Full', () => {
    const sections = projectSettingsSections(true);
    expect(sections.map(section => section.id)).toEqual(['general', 'telemetry', 'integrations', 'updates', 'storage', 'developer']);
    expect(sections.find(section => section.id === 'developer')?.items).toContain('developerTuning');
  });
  ...
  it('keeps every non Developer setting visible in Lite', () => {
    ...
    expect(items).toEqual(['language', 'units', 'telemetry', 'recording', 'discord', 'updates', 'storage']);
  });
});
