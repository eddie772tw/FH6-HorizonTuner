...
export function DriftCandidate({ run, setup, busy, createCandidate }: Props) {
  const { t } = useSettings();
  const [param, setParam] = useState('pressure.rear');
  const [baseVal, setBaseVal] = useState('32.0');
  const [candVal, setCandVal] = useState('32.5');
  const [hypothesis, setHypothesis] = useState('Increase rear tire pressure to induce slide initiation more progressively');
        ...
        <div className="row g-3">
          <div className="col-md-4">
            ...
            <select className="form-select" value={param} onChange={e => setParam(e.target.value)}>
              ...
              <option value="diff.acceleration">{t('Rear Diff Acceleration (%)')}</option>
            </select>
          </div>
          <div className="col-md-4">
            <label className="form-label">{t('Baseline Value (A)')}</label>
            <input type="number" step="0.5" className="form-control" value={baseVal} onChange={e => setBaseVal(e.target.value)} required />
          </div>
          <div className="col-md-4">
            <label className="form-label">{t('Candidate Value (B)')}</label>
            <input type="number" step="0.5" className="form-control" value={candVal} onChange={e => setCandVal(e.target.value)} required />
          </div>
        </div>
        <div>
