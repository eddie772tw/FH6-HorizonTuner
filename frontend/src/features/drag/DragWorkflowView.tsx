...
import { DragPrepare } from './DragPrepare';
import { DragResults } from './DragResults';
import { DragRunPanel } from './DragRunPanel';
import { documentsOf, type DragWorkflow } from './dragTypes';
import { useDragWorkflow } from './useDragWorkflow';

export function DragWorkflowView() {
  const { t } = useSettings();
  const { workflows, selectedId, select, documents, live, error, busy, perform } = useDragWorkflow();
  ...
  const handleCreate = async (body: unknown): Promise<DragWorkflow | null> => {
    const wf = await perform<DragWorkflow>('/workflows', body);
