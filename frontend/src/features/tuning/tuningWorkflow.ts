...
export function restoreWorkflowStep(value: unknown): number {
  ...
  if (state.schema === 'tuning-workflow/v1') return [0, 1, 5, 3, 2, 6][state.step ?? 0] || 1;
  return state.schema === 'tuning-workflow/v2' && TUNING_WORKFLOW_STEPS.some(item => item.number === state.step) ? state.step! : 1;
}
