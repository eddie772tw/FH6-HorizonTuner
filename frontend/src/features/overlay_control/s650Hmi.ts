...
/**
 * Converts the seven pre-HMI style ids into one HUD style plus a mode.
 * Non-S650 configurations are returned unchanged so this helper can be
 * applied at every config boundary without affecting other HUDs.
 */
export function normalizeS650HmiConfig<T extends { hudStyle?: string; s650Theme?: unknown }>(config: T): T {
  ...
  if (!legacyTheme && config.hudStyle !== 's650_hmi') {
    return config;
  }
  ...
  return {
    ...config,
    hudStyle: 's650_hmi',
    s650Theme: legacyTheme ?? (isS650HmiTheme(config.s650Theme) ? config.s650Theme : 'normal'),
  } as T;
}
