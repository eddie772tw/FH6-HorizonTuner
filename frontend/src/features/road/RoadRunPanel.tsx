...
interface Props { workflow: RoadWorkflow; setups: RoadSetup[]; selectedSetupId: string; selectSetup: (id: string) => void; live: RoadLive | null; busy: boolean; start: (body: unknown) => Promise<unknown>; stop: () => Promise<unknown> }
export function RoadRunPanel({ workflow, setups, selectedSetupId, selectSetup, live, busy, start, stop }: Props) {
  const { t } = useSettings();
  ...
  useEffect(() => { setConfirmed(false); setUnchanged(false); }, [selectedSetupId, active?.id]);
  const match = live?.fresh && live.identity?.ordinal === workflow.identity.ordinal && live.identity?.performanceIndex === workflow.identity.performanceIndex && live.identity?.drivetrain === workflow.identity.drivetrain;
  const begin = async () => {
  ...
  return <section className="glass-panel p-4">
    ...
    {active ? <>
      ...
      <p className="small text-body-secondary">{t('Start recording before starting the event. No seasonal pressure adjustment is applied. After recording, return here to review the saved result.')}</p>
      <button className="btn btn-primary" disabled={busy || !confirmed || !selected || !match} onClick={() => void begin()}>{t('Start Road recording')}</button>
