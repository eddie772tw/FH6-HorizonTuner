const candidateUnits: Record<string, string[]> = {
  pressure: ['psi', 'bar', 'kPa'], spring: ['kgf/mm', 'lb/in', 'N/mm'], height: ['cm', 'in'], diff: ['%'],
  arb: ['slider'], rebound: ['slider'], bump: ['slider'], camber: ['deg'], toe: ['deg'], caster: ['deg'], gearing: ['ratio'],
};
export const candidateParameterUnits = (parameter: string): string[] => candidateUnits[parameter.split('.')[0]] || [];
export function candidateParameters(savedKeys: string[]): string[] {
  // Native snapshots already exclude controls unavailable on this vehicle.
  // Empty legacy baselines still permit an explicit in-game readback.
  const keys = savedKeys.length ? savedKeys : ['pressure.front', 'pressure.rear', 'arb.front', 'arb.rear', 'spring.front', 'spring.rear', 'height.front', 'height.rear', 'rebound.front', 'rebound.rear', 'bump.front', 'bump.rear', 'diff.front.acceleration', 'diff.front.deceleration', 'diff.rear.acceleration', 'diff.rear.deceleration', 'diff.center'];
  return keys.filter(key => candidateParameterUnits(key).length > 0);
}
