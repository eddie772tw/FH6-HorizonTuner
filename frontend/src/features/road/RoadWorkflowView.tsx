...
function RoadWorkflowContent({ recommendation, carId }: Props) {
  ...
  const { selectedWorkflowId, setStep } = validation;

  useEffect(() => {
    if (state.live?.activeRun?.workflowId === selectedWorkflowId) setStep('drive');
    else if (summaryCount) setStep('results');
  }, [selectedWorkflowId, setStep, state.live?.activeRun?.id, summaryCount]);
  ...
  const perform = async <T,>(path: string, body: unknown): Promise<T | null> => {
    ...
    return validation.finishOperation(operationId, result !== null) ? result : null;
  };
  ...
  return <div className="d-flex flex-column gap-3 overflow-auto p-2" style={{ minHeight: 0 }}>
    ...
    {validation.step === 'prepare' && <RoadPrepare live={state.live} busy={!recommendation || state.busy || state.live?.identity?.ordinal !== Number(carId)}
    ...
    {validation.step === 'drive' && work && <RoadRunPanel workflow={work} setups={setups} selectedSetupId={selectedSetupId} selectSetup={validation.setSetupId} live={state.live} busy={state.busy}
    ...
    {validation.step === 'results' && work && <RoadResults key={work.id} documents={state.documents} busy={state.busy} selection={validation.resultSelection} candidateDraft={validation.candidateDraft}
      onSelectionChange={validation.setResultSelection} onCandidateDraftChange={validation.setCandidateDraft} perform={perform} prefix={prefix}
