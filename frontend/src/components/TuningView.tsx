...
const TuningView: React.FC<{ setActiveTab?: (tab: any) => void }> = ({ setActiveTab }) => {
  ...
  const applySuggestedGearing = () => {
    ...
    let platformRpm = pTorque + range * 0.5; // NA/SC
    ...
    for (let i = 0; i < numGears - 1; i++) {
      let targetYi = 0;
      if (gearingDiscipline === 'Rally') {
        targetYi = pTorque;
      } else if (gearingDiscipline === 'Drift') {
        if (i === 0) targetYi = maxRpm * 0.5;
        else targetYi = platformRpm;
      } else {
        // GT
        const y1 = maxRpm * 0.65;
        if (numGears <= 2) targetYi = pPower;
        else targetYi = y1 + (pPower - y1) * Math.sqrt(i / (numGears - 2));
      }
  ...
  const getTheoreticalYi = (i: number, numGears: number) => {
    ...
    const y1 = tuning.gearing.maxRpm * 0.65;
    if (numGears <= 2) return pPower;
    return y1 + (pPower - y1) * Math.sqrt(i / (numGears - 2));
  };
            ...
            {activeSubTab === 'Gearing' && (
              <div style={{ display: 'flex', gap: '2rem', height: '100%' }}>
                ...
                <div style={{ flex: 2, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  ...
                  <div style={{ flex: 1, maxHeight: '450px', minHeight: '300px' }}>
