...
## 2024-08-24 - Optimize 60Hz DOM updates with element pooling and property caching
**Learning:** In high-frequency JavaScript rendering loops (e.g. 60Hz telemetry HUD overlays), modifying the DOM indiscriminately via `innerHTML = ""` or unconditional `textContent` / `style` updates causes excessive layout recalculations, style invalidations, and Garbage Collection (GC) pauses. Creating new elements every frame drastically impacts rendering performance and can lead to frame drops.
**Action:** When updating the DOM in tight loops, implement object pooling for dynamically generated elements (e.g., G-force markers) instead of recreating them, and always check if a property (like `textContent` or `style.height`) actually needs to change before updating it (e.g., `if (el.textContent !== newVal) el.textContent = newVal`).
