...
## 2024-11-26 - Inline Array Literals and Checks over Native Array Methods
**Learning:** In high-frequency React frontend telemetry processing loops (e.g., executing math for every tick of data to feed `useMemo` hooks), native helper methods like `Array.from()` (with inline callbacks) and `Array.prototype.some()` introduce significant overhead. They allocate closures, iterators, and temporary arrays, forcing the Garbage Collector to run frequently and causing micro-stutters.
**Action:** Replace `Array.from()` with direct array literals (e.g., `[val0, val1, val2, val3]`). Replace iteration-based checks like `.some()` with explicitly mapped manual index comparisons (e.g., `val[0] === null || val[1] === null`). This eliminates allocations completely in these hot paths.
