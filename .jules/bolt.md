...
## 2024-03-XX - Typed Arrays for Double Iteration Caching
**Learning:** Iterating large telemetry arrays twice (once with `.forEach` for math/max calculations, and again with `.map` to format the result objects) creates redundant CPU overhead and massive object allocation inside `.map` during high-frequency React component updates.
**Action:** Replace `.forEach` and `.map` with standard `for` loops. Use a typed array (e.g. `Float64Array`) in the first loop to cache intermediate computation results, allowing the second loop to skip the heavy math operations entirely while constructing the result objects.
