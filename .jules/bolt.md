...
## 2024-11-26 - Eliminating list allocations and function call overhead in Python telemetry matching loops
**Learning:** In backend data processing functions (like `spatial_points` comparing telemetry streams), looping over small fixed-length arrays using list comprehensions (e.g., `[wheel_value(p, key, i) for i in range(4)]`) and `all()` checks on every iteration allocates many intermediate objects and heavily taxes the CPU with function call overhead.
**Action:** Replace small fixed-length loops and `all()` checks with manual array unrolling (e.g., extracting `p.get("TireSlipAngle")` once, indexing elements `0..3` manually, and using explicit `if finite(...)` scalar checks). This improves matching loop execution speed by 3-4x.
