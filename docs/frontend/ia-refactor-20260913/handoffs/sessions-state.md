...
Changed: see the paths listed below
...
`SessionsRuntime` is the sole race-end observer after root mounts it. It captures the recorder identity while racing, including a read of `/api/analysis/status` at race start. On `IsRaceOn: 1 -> 0`, it calls `waitForCompletedRaceSession` with a bounded maximum of eight attempts at 500 ms cadence. A successful result requires all three facts for the same captured identity:
...
111 files, 733 tests passed

cmd /c "pnpm -C frontend run build"
passed; Full and Lite Vite entries compiled
```

The two new test files are pure/async Vitest coverage only: source separation, stale response rejection, abort/disposal, monotonic intent consumption, completion identity, list/data readiness, bounded retry, and cancelled observer behavior. They do not mount React or assert DOM details.
