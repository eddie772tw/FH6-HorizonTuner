...
日期：2026-09-20。狀態：`proposed`。Owner：Coordinator/root。
...
- 整合 head：`4e24fac07309b5fe71e76f28cf3f3a1a7e37115f`，已推送到同名 remote branch，工作樹乾淨。
- 使用者目前開啟的 `C:\Users\eddie\.codex\worktrees\a945\FH6-HorizonTuner` 是 detached `568da20`，只有未追蹤的準備文件；它不是 W2/W3 產品 head。W4 必須從上述 `4e24fac` 建立新 worktree/branch，不能在 detached 工作樹直接接續。
...
| Frontend gate | 通過 | `pnpm -C frontend exec tsc --noEmit`、`pnpm -C frontend run test`（125 files / 884 tests）、`pnpm -C frontend run build`、`git diff --check` 均在整合 head 通過。 |
| Browser shell smoke | 局部通過 | Full browser 已實際開啟 Sessions、HUD、App Menu → Updates；Sessions 觀察到 empty review prompt 與 `Failed to fetch` backend-disconnected 狀態。這證明掛載與入口，不證明 backend success、save、download 或 real game。 |
...
2. `TuneSessionBoundary` 與 `RoadValidationBoundary` 仍存在。W4 要先盤點正式 Full/Lite provider 的唯一 owner，再移除重複 fallback；不能只因檔名看起來像 adapter 就直接刪除。
3. `currentStep` / `setCurrentStep` 等舊相容 props 仍需查明是否仍有真實 consumer。若保留，必須記錄保留理由與最晚 gate；若移除，需由既有 Tune restore/readiness 測試及瀏覽器往返證據支撐。
4. 新 Road/HUD 文案目前走既有 `t()` fallback；locale catalog 補齊是獨立 maintenance，W4 只登記 key 清單與不影響功能的處理方式。
5. X1 resource inventory、X2 Full/Lite keyboard/theme/layout matrix、X3 三次成對效能、X4 最終組合文件尚未形成新的 W4 evidence record。
