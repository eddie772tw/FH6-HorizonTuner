...
工作流導覽與單向計算已另行落實於[六階段規劃](tuning-workflow-dependencies-20260910.md)：輪胎基準 → 底盤平台 → 定位 → 動力／傳動 → 實際設定驗證；各機械頁依資料開放。本文的跨場比較、持久化版本及賽後回饋仍為提案。後續使用 `setup/tires/chassis/alignment/powertrain/validation` 語意階段；`Step5TelemetryCalibration.tsx` 是保留的程式檔名，目前對應第 6 階段。

2026-09-10 使用者回饋後續：[流程簡化與遙測輔助方案](tuning-workflow-simplification-proposal-20260910.md)範圍限定 Road，補充按需輸入、季節／配方替代邊界、局部確認與最小 A/B 流程。Road 一般模式以改善現有設定為預設，不強制 A→S→SD→A；該區塊保留為研究範本。後續第一版產品實作與本地驗證見 [Road 實作紀錄](road-workflow-implementation-20260910.md)，本提案其餘內容保留為設計基線。
...
1. **準備**：預設改善現有設定；觀察不要求完整靜態資料或引擎量測，生成候選時才提示依賴欄位。從零調校保留整套公式路徑。MR未知時保留未知／輪端目標，不要求使用者猜一個值來解鎖結論。
2. **建立比較**：選既有確認設定作A，一般模式只建立一項變更的B，核對目標參數及未變條件，並標明局部／整套確認。S／SD整套模型比較保留於進階。自動列出實際改動，使用者確認遊戲讀回後凍結候選；若設定差超出計畫，保留偏差而非靜默改設計。
3. **執行場次**：一般模式先記錄A，有候選才比較A→B；已有適當A可重用但顯示時間差與相容性。單次比較只提供描述性方向，需要更強證據再引導A回訪或跨場重複。A→S→SD→A保留研究用途。允許只跑一場、暫停或保留基準，不強迫為完成精靈而額外開賽。
