...
* **Road 比較助手（本分支第一版）**：調校入口採「確認車輛與賽事 → 開一趟 → 結果與下一步」。觀察現有設定不要求引擎、季節或輪胎配方；建立初值時按領域補資料，採無季節偏移的初始估計。
  - 保存不可覆寫的 A/B、場次、完賽讀回與報告；離線及重啟後可查閱，回復 A 或保留 B 為下一輪新基準。新場仍需遊戲值確認及新遙測，沒有自動套用遊戲設定。
  - 比較整場時間、起始熱狀態及相近位置／操控的滑移與胎溫；缺資料會限制結論。第一版提供描述性證據，沒有通用最佳胎壓或因果信心評分。
  - 本地與合成遙測已驗證；真實 Road 效果、跨車門檻及新手完成率仍待測量。操作、資料契約與限制見 [Road 實作紀錄](docs/road-workflow-implementation-20260910.md)。詳細六領域工作台仍可開啟。
